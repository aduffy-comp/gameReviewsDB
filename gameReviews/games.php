...
<html lang="en">
    <head>
        ...
        <title>Latest Games - Mod Reviews</title>
    ...
    <body>
        <div class="container">
            ...
            <nav class="navbar fixed-top navbar-expand-lg navbar-light bg-light">
                ...
                <div class="collapse navbar-collapse" id="navbar-content">
                    ...
                    <div class="navbar-nav nav-fill w-100 align-items-start"> <!--nav-fill, w-100 and align-items-start stretch the elements to fill the navbar-->
                        ...
                        <a class="nav-item nav-link active px-2" href="games.php">Games</a>
            ...
            <div class="row">
                <div class="col">
                    <h1 class="display-4">Games List</h1>
                </div>
            </div>
            <br>
            ...
            <form action="gameinfo.php" method="GET">
            ...
            <!-- latest 6 games appear here -->
            <div class="row">
                <h2>Latest Games</h2>
            </div>
            <br>
            <div class="row">
                <!--Game summary retriever script-->
                <?php
                    //only return the 6 most recent games
                    //aliases required as multiple tables share the 'name' column title
                    $sql = "SELECT game.name AS gamename, game.id, game.releaseDate, game.classification, game.criticreview, developer.name AS devname, publisher.name AS pubname
                    FROM ((game
                    INNER JOIN developer ON game.developerID = developer.id) INNER JOIN publisher ON game.publisherID = publisher.id) ORDER BY game.releaseDate DESC LIMIT 6";
                    ...
                    if($results = mysqli_query($con, $sql)) {
                        if(mysqli_num_rows($results) > 0) {
                            while($row = mysqli_fetch_array($results)) {
                                ...
                                echo("<h5 class='card-title'>" . $row['gamename'] . " (" . $row['classification'] . ") - " . $row['criticreview'] . "/100</h5>");
                                echo "<p class='card-text'> Released on " . $row['releaseDate'] . "</p>";
                                echo "<p class='card-text'> Developed by " . $row['devname'] . "</p>";
                                echo "<p class='card-text'> Published by " . $row['pubname'] . "</p>";
                                echo("<a href='gameinfo.php?name=" . $row['gamename'] . "' class='btn btn-primary'>Details & Mods</a>");
                                echo("</div>");
                                echo("</div>");
                            }
                        }
		...
		<!--Typeahead interface for this page-->
        <script type="text/javascript" src="gamesearch.js"></script>
    </body>
</html>